...
def inclination(date_decimal, alon, alat):
    ...
    coeffs = load_coeffs(date_decimal)

    # Set altitude to 300 km
    aalt = np.zeros((alon.shape)) + 300.0
    ...
    # fix poles
    #    inc = np.where(np.abs(alat)==90.0, np.abs(inc) * np.sign(alat), inc)
...
def gg_to_geo(h, gdcolat):
    ...
    plrad = eqrad * (1.0 - flat)  # polar radius
...
def geo_to_gg(radius, theta):
    ...
    z0 = b2 * z / (a * V)
    height = U * (1.0 - b2 / (a * V))
    beta = 90.0 - np.degrees(np.arctan2(z + ep2 * z0, r))
...
def synth_values(coeffs, radius, theta, phi, nmax=None, nmin=1, grid=False):
    ...
    try:
        ...
        raise ValueError(
            "".join(
                [
                    "Cannot broadcast grid shapes (excl. last ",
                    "dimension of coeffs):\nradius: ",
                    repr(radius.shape),
                    "\ntheta: ",
                    repr(theta.shape),
    ...
    for n in range(nmin, nmax + 1):
        ...
        for m in range(1, n + 1):
            B_radius += (
                ...
                * Pnm[n, m]
                * r_n
                * (coeffs[..., num] * cos_mp[m] + coeffs[..., num + 1] * sin_mp[m])
            )
            B_theta += (
                -Pnm[m, n + 1]
                * r_n
            ...
            B_phi += (
                ...
                * div_Pnm
                * r_n
                * (coeffs[..., num] * sin_mp[m] - coeffs[..., num + 1] * cos_mp[m])
            )
            num += 2
...
def legendre_poly(nmax, theta):
    ...
    for n in range(2, nmax + 1):
        n = int(n)
        Pnm[0, n + 1] = -np.sqrt((n * n + n) / 2.0) * Pnm[n, 1]
        Pnm[1, n + 1] = (
            np.sqrt(2.0 * (n * n + n)) * Pnm[n, 0]
        ...
        Pnm[n, n + 1] = np.sqrt(2.0 * n) * Pnm[n, n - 1] / 2.0
...
def inc2mlat(inc: float) -> float:
    return np.degrees(np.arctan(np.tan(np.radians(inc)) * 0.5))
